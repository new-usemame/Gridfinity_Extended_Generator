...
import { SavePromptModal } from '../SavePromptModal/SavePromptModal';
...
export function ExportButtons({ stlUrl, scadContent, filename, isConfigSaved = true, user, onSaveRequest, onAuthRequest }: ExportButtonsProps) {
  ...
  const downloadSTL = () => {
    ...
    // Show save prompt after download if config is not saved
    if (!isConfigSaved && (onSaveRequest || onAuthRequest)) {
...
export function MultiSegmentExportButtons({ result, splitInfo, baseplateConfig, isConfigSaved = true, user, onSaveRequest, onAuthRequest }: MultiSegmentExportButtonsProps) {
  ...
  const downloadAllSequentially = async () => {
    ...
    // Show save prompt after all downloads complete if config is not saved
    if (!isConfigSaved && (onSaveRequest || onAuthRequest)) {
      setTimeout(() => {
        setShowSavePrompt(true);
      }, 100);
